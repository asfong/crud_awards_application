...
//var mysql       = require('./db_localhost.js')  // for localhost database.
var sequelize = require('sequelize');
let User = require('./models/User');
require('./routes/api');
var hookLocalStrategy = require('./services/passportLocal');
...
app.use(function(req, res, next) {
 ...
 // for FLIP
 res.header("Access-Control-Allow-Origin", "http://web.engr.oregonstate.edu");
...
app.get('/giverschart', (req, res) => {
    mysql.pool.query(`SELECT G.lname AS 'last', CONCAT(G.fname, ' ', G.lname) AS 'giver', SUM(case when aid = 1 then 1 else 0 end) eotm, SUM(case when aid = 2 then 1 else 0 end) eotw  FROM award_winner INNER JOIN user_account G ON G.id = gid GROUP BY giver ORDER BY last ASC`, (err, result) => {
      if(err) throw err;
    	console.log('./awardgivers: database found...');
      res.send(JSON.stringify(result));
    });
...
app.get('/receiverschart', (req, res) => {
    mysql.pool.query(`SELECT R.lname AS 'last', CONCAT(R.fname, ' ', R.lname) AS 'receiver', SUM(case when aid = 1 then 1 else 0 end) eotm, SUM(case when aid = 2 then 1 else 0 end) eotw  FROM award_winner INNER JOIN user_account R ON R.id = rid GROUP BY receiver ORDER BY last ASC`, (err, result) => {
      if(err) throw err;
    	console.log('./awardgivers: database found...');
      res.send(JSON.stringify(result));
    });
...
//     mysql.pool.query(`SELECT COUNT(*) AS 'count', MONTH(date) AS 'month', YEAR(date) AS 'year' FROM award_winner GROUP BY month, year ORDER BY date ASC`, (err, result) => {
//       if(err) throw err;
//     	console.log('./awardgivers: database found...');
//       res.send(JSON.stringify(result));
//     });
...
// awards by month
app.get('/monthlyawards', (req, res) => {
    mysql.pool.query(`SELECT COUNT(*) AS 'count', MONTH(date) AS 'month', YEAR(date) AS 'year', date FROM award_winner GROUP BY month, year ORDER BY date ASC`, (err, result) => {
      if(err) throw err;
    	console.log('./awardgivers: database found...');
      res.send(JSON.stringify(result));
    });
...
// -- AWARD ROUTES --
// list all awards
// Note: Edited FROM 'team_kang.award_winner' to just FROM 'award_winner' to access table
// In local database, I used team_kang.award_winner
